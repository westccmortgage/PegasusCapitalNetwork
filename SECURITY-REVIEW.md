...
Scope: migrations **067–070**, the `intelligence-*` Netlify functions, the
...
`public.is_admin_user()`; the import functions run with the service role only
after independently verifying the caller's admin JWT; the two write RPCs are
`service_role`-EXECUTE-only; the document bucket is private with admin-only
storage RLS.** Page redirects and hidden links are convenience, not the
boundary — the boundary is RLS + the function-level admin check.
...
## 3. All grants / revokes introduced (067–070)
...
- `authenticated` non-admins evaluate `is_admin_user()` → `false` → denied on
  read and write (both `using` and `with check`).
- Only the `service_role` (used by the functions) bypasses RLS, and only after
  the function's own admin check.
...
4. **Page gates** (UX only) — `admin-intelligence.html` runs a store check
   (redirect non-admins) **and** a live `PegIntelAPI.verifyAdmin()` DB re-check
   before rendering. Bypassing both still yields no data (layer 1).
5. **Routing/headers** — `netlify.toml` serves the page `no-store` +
   `X-Robots-Tag: noindex,nofollow`; page `<meta robots noindex>`; no sitemap
   entry; sidebar/account-menu links render for admins only.
...
migrations 067–070 applied there ONLY. Do not run against production. You need:
one admin account, one ordinary (non-admin) member account, and the anon key.

### A. Migration & schema
1. Apply `067 → 068 → 069 → 070` in order in the staging SQL editor. Each must
   succeed and be **re-runnable** (run 068–070 a second time → no error).
