    ...
    host.innerHTML =
      '<div class="pit-grid2"><div>' +
        '<div class="pit-panel"><h3>Upload the daily workbook</h3>' +
          '<div class="pit-drop" id="pitDrop" onclick="document.getElementById(\'pitFile\').click()">Drop a .xlsx or .csv here or click to choose<br><span style="font-size:11px">Every file goes through Detect → Map → Normalize → Review. Native templates skip mapping automatically. Max 4MB.</span></div>' +
          '<input type="file" id="pitFile" accept=".xlsx,.csv" style="display:none" onchange="PegIntel.mapChosen(this)">' +
          '<div style="margin-top:10px;display:flex;gap:8px;align-items:center"><button class="btn btn-ghost btn-sm" onclick="document.getElementById(\'pitFileStrict\').click()">Strict native importer (advanced)</button><span class="pit-meta">Bypass the mapper for an exact native workbook.</span></div>' +
          '<input type="file" id="pitFileStrict" accept=".xlsx" style="display:none" onchange="PegIntel.pickFile(this)">' +
