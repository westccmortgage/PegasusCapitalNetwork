// ============================================================================
// PEGASUS CAPITAL INTELLIGENCE — read one import batch + its rows (admin only)
// POST { batch_id, only? }   only: 'conflict' | 'invalid' | 'all' (default all)
//
// Used by the Import Center to render the preview/conflict screen and to
// download an error report. Read-only.
// ============================================================================
"use strict";
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { data: batch, error: bErr } = await supabase.from("pci_import_batches")
    ...
    let q = supabase.from("pci_import_rows")
    ...
    console.error("[intelligence-import-batch] ERROR:", err.message);
