// ============================================================================
// PEGASUS CAPITAL INTELLIGENCE — import commit (admin only)
// POST { batch_id, resolutions? }  with Authorization: Bearer <jwt>
//
// resolutions: { "<row uuid>": "apply" | "keep" } for previewed conflicts.
// Delegates to the transactional RPC pci_commit_import_batch (069) so a
// partial import can never corrupt the database. The RPC also enforces the
// backstop: Verified data is never downgraded without an explicit "apply".
// ============================================================================
"use strict";
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("pci_commit_import_batch", {
      p_batch_id: batchId, p_admin_id: uid, p_resolutions: resolutions,
    });
    if (error) return resp(500, { ok: false, error: "commit failed (apply migration 069?): " + error.message });
    console.log("[intelligence-import-commit] batch=" + batchId + " by admin=" + uid + " → " + JSON.stringify(data));
    return resp(data && data.ok ? 200 : 422, data);
  } catch (err) {
    console.error("[intelligence-import-commit] ERROR:", err.message);
